...
DG.QuizMaker = new Class( {
	...
	validEvents : ['start','sendanswer', 'correctanswer','wronganswer', 'finish','missinganswer'],
	...
	_addAnsweringOptions : function() {
		...
		for(var i=0;i<options.length;i++) {
			...
			checkbox.name = 'dg-quiz-options';
			checkbox.id = id;
			checkbox.type = isMulti ? 'checkbox' : 'radio';
			checkbox.set('value', option);
			el.adopt(checkbox);

			var label = new Element('label');
			label.setAttribute('for', id);
			label.set('html', option);
			el.adopt(label);
	...
	_getAnswersFromForm : function() {
		var ret = [];
		var els = $(this.html.el).getElements('input');
